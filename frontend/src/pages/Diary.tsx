...
function EntryCard({ e, reload }: { e: DiaryEntry; reload: () => void }) {
  ...
  async function del() {
    if (!(await confirm({ message: 'Eintrag löschen?', danger: true, confirmLabel: 'Löschen' }))) return;
    try {
...
export default function Diary() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        title="Bautagebuch"
        subtitle={`${list.length} Eintrag/Einträge`}
        actions={
          <div className="flex gap-2">
            ...
            <Button onClick={() => setShowAdd(true)}>+ Eintrag</Button>
          </div>
        }
      />
      {error && <ErrorBox>{error}</ErrorBox>}
      {list.length === 0 ? (
        <EmptyState>Noch keine Einträge. Dokumentiere täglich Fortschritt, Wetter, Gewerk und besondere Vorkommnisse — das ist ein wichtiger Nachweis.</EmptyState>
      ) : (
      ...
      <Modal open={showAdd} onClose={() => setShowAdd(false)} title="Tagebuch-Eintrag" busy={busy}>
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Gewerk">
              <Input value={form.trade} onChange={(e) => setForm((f) => ({ ...f, trade: e.target.value }))} placeholder="z. B. Estrich" />
            </Field>
            <Field label="Wetter">
              <Input value={form.weather} onChange={(e) => setForm((f) => ({ ...f, weather: e.target.value }))} placeholder="sonnig, 18 °C" />
            </Field>
